 ...
 *
 * @file       pios_ms5637.c
 * @author     Librepilot, http://www.librepilot.org Copyright (C) 2017.
 *             The OpenPilot Team, http://www.openpilot.org Copyright (C) 2012.
 * @brief      MS5637 Pressure Sensor Routines
...
#define PIOS_MS5637_OVERSAMPLING   oversampling

// Option to change the interleave between Temp and Pressure conversions
// Undef for normal operation
#define PIOS_MS5637_SLOW_TEMP_RATE 20
#ifndef PIOS_MS5637_SLOW_TEMP_RATE
#define PIOS_MS5637_SLOW_TEMP_RATE 1
#endif
// Running moving average smoothing factor
#define PIOS_MS5637_TEMP_SMOOTHING 10
//

#define PIOS_MS5637_I2C_RETRIES    5

/* Local Types */
typedef struct {
    uint16_t C[6];
} MS5637CalibDataTypeDef;

typedef enum {
    MS5637_CONVERSION_TYPE_None = 0,
    MS5637_CONVERSION_TYPE_PressureConv,
    MS5637_CONVERSION_TYPE_TemperatureConv
} ConversionTypeTypeDef;

typedef enum {
    MS5637_FSM_INIT = 0,
    MS5637_FSM_TEMPERATURE,
    MS5637_FSM_PRESSURE,
    MS5637_FSM_CALCULATE,
} MS5637_FSM_State;

/* Glocal Variables */
ConversionTypeTypeDef CurrentRead = MS5637_CONVERSION_TYPE_None;

/* Local Variables */
MS5637CalibDataTypeDef CalibData;
...
static int32_t PIOS_MS5637_Read(uint8_t address, uint8_t *buffer, uint8_t len);
static int32_t PIOS_MS5637_WriteCommand(uint8_t command);
static uint32_t PIOS_MS5637_GetDelay();
static uint32_t PIOS_MS5637_GetDelayUs();
...
static const struct pios_ms5637_cfg *dev_cfg;
...
bool PIOS_MS5637_driver_Test(uintptr_t context);
void PIOS_MS5637_driver_Reset(uintptr_t context);
void PIOS_MS5637_driver_get_scale(float *scales, uint8_t size, uintptr_t context);
void PIOS_MS5637_driver_fetch(void *, uint8_t size, uintptr_t context);
bool PIOS_MS5637_driver_poll(uintptr_t context);

const PIOS_SENSORS_Driver PIOS_MS5637_Driver = {
    .test      = PIOS_MS5637_driver_Test,
    .poll      = PIOS_MS5637_driver_poll,
    .fetch     = PIOS_MS5637_driver_fetch,
    .reset     = PIOS_MS5637_driver_Reset,
    .get_queue = NULL,
    .get_scale = PIOS_MS5637_driver_get_scale,
    .is_polled = true,
};
/**
 * Initialise the MS5637 sensor
 */
int32_t ms5637_read_flag;
void PIOS_MS5637_Init(const struct pios_ms5637_cfg *cfg, int32_t i2c_device)
{
    ...
    if (cfg) {
        ...
        conversionDelayMs = PIOS_MS5637_GetDelay();
        conversionDelayUs = PIOS_MS5637_GetDelayUs();
    ...
    if (PIOS_MS5637_WriteCommand(MS5637_RESET) < 0) {
        return;
    }
    ...
    for (int i = 0; i < 6; i++) {
        if (PIOS_MS5637_Read(MS5637_CALIB_ADDR + i * 2, data, 2) != 0) {
            return;
        }
 ...
 */
int32_t PIOS_MS5637_StartADC(ConversionTypeTypeDef Type)
{
    ...
    if (Type == MS5637_CONVERSION_TYPE_TemperatureConv) {
        if (PIOS_MS5637_WriteCommand(MS5637_TEMP_ADDR + oversampling) != 0) {
            return -2;
        }
    } else if (Type == MS5637_CONVERSION_TYPE_PressureConv) {
        if (PIOS_MS5637_WriteCommand(MS5637_PRES_ADDR + oversampling) != 0) {
            return -2;
        }
    }
 ...
 */
int32_t PIOS_MS5637_ReadADC(void)
{
    if (!sensorIsAlive) { /* if sensor is not alive, don't bother, wait for next poll to try to reinitialize */
        return -2;
    }
    ...
    if (CurrentRead == MS5637_CONVERSION_TYPE_None) {
        return -2;
    }
    ...
    if (CurrentRead == MS5637_CONVERSION_TYPE_TemperatureConv) {
        /* Read the temperature conversion */
        if (PIOS_MS5637_Read(MS5637_ADC_READ, Data, 3) != 0) {
            return -2;
        }
        ...
        if (FilteredTemperature != INT32_MIN) {
            FilteredTemperature = (FilteredTemperature * (PIOS_MS5637_TEMP_SMOOTHING - 1)
                                   + Temperature) / PIOS_MS5637_TEMP_SMOOTHING;
        } else {
 ...
 */
static float PIOS_MS5637_GetTemperature(void)
{
 ...
 */
static float PIOS_MS5637_GetPressure(void)
{
...
static int32_t PIOS_MS5637_Read(uint8_t address, uint8_t *buffer, uint8_t len)
{
    ...
    for (uint8_t retry = PIOS_MS5637_I2C_RETRIES; retry > 0; --retry) {
...
/**
 * Writes one or more bytes to the MS5637
...
int32_t PIOS_MS5637_Test()
{
    ...
    cur_value = Pressure;
    PIOS_MS5637_StartADC(MS5637_CONVERSION_TYPE_PressureConv);
    PIOS_DELAY_WaitmS(10);
    PIOS_MS5637_ReadADC();
...
void PIOS_MS5637_Register()
{
    PIOS_SENSORS_Register(&PIOS_MS5637_Driver, PIOS_SENSORS_TYPE_1AXIS_BARO, 0);
}

bool PIOS_MS5637_driver_Test(__attribute__((unused)) uintptr_t context)
{
    return true; // !PIOS_MS5637_Test();
}

void PIOS_MS5637_driver_Reset(__attribute__((unused)) uintptr_t context) {}

void PIOS_MS5637_driver_get_scale(float *scales, uint8_t size, __attribute__((unused))  uintptr_t context)
{
    PIOS_Assert(size > 0);
    scales[0] = 1;
}

void PIOS_MS5637_driver_fetch(void *data, __attribute__((unused)) uint8_t size, __attribute__((unused)) uintptr_t context)
{
    PIOS_Assert(data);
    memcpy(data, (void *)&results, sizeof(PIOS_SENSORS_1Axis_SensorsWithTemp));
}

bool PIOS_MS5637_driver_poll(__attribute__((unused)) uintptr_t context)
{
    static uint8_t temp_press_interleave_count = 1;
    static MS5637_FSM_State next_state = MS5637_FSM_INIT;

    if (!sensorIsAlive) { // try to reinit
        PIOS_MS5637_Init(0, 0);
    }

    int32_t conversionResult = PIOS_MS5637_ReadADC();
    ...
    if (__builtin_expect(conversionResult == -1, 1)) {
        ...
        next_state = MS5637_FSM_INIT;
    }
    switch (next_state) {
    case MS5637_FSM_INIT:
    case MS5637_FSM_TEMPERATURE:
        PIOS_MS5637_StartADC(MS5637_CONVERSION_TYPE_TemperatureConv);
        next_state = MS5637_FSM_PRESSURE;
        return false;

    case MS5637_FSM_PRESSURE:
        PIOS_MS5637_StartADC(MS5637_CONVERSION_TYPE_PressureConv);
        next_state = MS5637_FSM_CALCULATE;
        return false;

    case MS5637_FSM_CALCULATE:
        temp_press_interleave_count--;
        if (!temp_press_interleave_count) {
            temp_press_interleave_count = PIOS_MS5637_SLOW_TEMP_RATE;
            PIOS_MS5637_StartADC(MS5637_CONVERSION_TYPE_TemperatureConv);
            next_state = MS5637_FSM_PRESSURE;
        } else {
            PIOS_MS5637_StartADC(MS5637_CONVERSION_TYPE_PressureConv);
        ...
        results.temperature = PIOS_MS5637_GetTemperature();
        results.sample = PIOS_MS5637_GetPressure();
        return true;
